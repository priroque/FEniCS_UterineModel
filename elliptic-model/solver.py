...
# -----------------------------------------------------------------------------
def sigma(u, p, vg_l, vg_c, f0, s0, prm_HO):
    """
    ...
    ----------
        ...
        p : Pressure term.
        vg_l, vg_c : Growth parameters (longitudinal, circumferential).
    ...
    # Growth tensor
    Fg = I + (vg_l - 1.0) * outer(f0,f0) + (vg_c - 1.0) * outer(s0,s0)        
    ...
    # Material parameters
    c, c_1, c_2, c_3, c_4, zeta1 = prm_HO
    ...
    Sel = ( c*I - p*Je*inv(Ce).T 
           + 2.0 * zeta1 * c_1 * subplus(I4 - 1.0) 
           * exp(c_2 * (subplus(I4 - 1.0) ** 2)) * outer(f0,f0)
           + 2.0 * (1.0 - zeta1) * c_3 * subplus(I6 - 1.0)
           * exp(c_4 * (subplus(I6 - 1.0) ** 2)) * outer(s0,s0) )
...
# -----------------------------------------------------------------------------
def UterusSolve(file_results, mesh, bdry, f0, s0,  
                prm_Holzapfel, prm_GrowthL, prm_GrowthC, IUP, nsteps):
    """
    ...
    # Function space formed by elements (u, p, vg_l, vg_c)
    W = FunctionSpace(mesh, MixedElement(P2v, P1, P1, P1))
    ...
    (u, p, vg_l, vg_c) = split(w)
    (v, q, wg_l, wg_c) = split(psiW)
    ...
    # Material constants
    c, c_1, c_2, c_3, c_4, zeta1 = prm_Holzapfel

    # Subplus function: returns variable if >=0, else 0
    subplus = lambda var : conditional(ge(var, 0.0), var, 0.0)

    # Second Piola-Kirchhoff stress tensor for elastic deformation
    Se = ( c*I - p*Je*inv(Ce).T 
           + 2.0 * zeta1 * c_1 * subplus(I4 - 1.0) 
           * exp(c_2 * (subplus(I4 - 1.0) ** 2)) * outer(f0,f0)
           + 2.0 * (1.0 - zeta1) * c_3 * subplus(I6 - 1.0)
           * exp(c_4 * (subplus(I6 - 1.0) ** 2)) * outer(s0,s0) )
    ...
    bc1 = DirichletBC(W.sub(0), zerov, bdry, 10)   # Fix base-cervix - marker 10
    bcs = [bc1]
    ...
    def heav_tanh(x, lcrit):
        k = 150.0
        y = (1.0/2.0)*(1.0+(exp((2.0*k)*(x-lcrit))-1.0)/(exp((2.0*k)*(x-lcrit))+1.0))
        return y
    ...
    with open(f'./results_measures/out_elliptic_t_press_volIU_volUter.txt', 'w') as f_out:
        f_out.write("")
    ...
    while (step < nsteps):
        ...
        uaux, paux, vglaux, vgcaux = w.split(deepcopy = True)
        ...
        print("u MIN MAX",uaux.vector().get_local().min(),uaux.vector().get_local().max())
        print("VG_l MIN MAX",vglaux.vector().get_local().min(),vglaux.vector().get_local().max())
        print("VG_c MIN MAX",vgcaux.vector().get_local().min(),vgcaux.vector().get_local().max())
        ...
        Fe, sig_e, sig_t, Ct = sigma(uaux, paux, vglaux, vgcaux, f0, s0, prm_Holzapfel)  
        ...
        with open(f'./results_measures/out_elliptic_t_press_volIU_volUter.txt', 'a') as f_out:
